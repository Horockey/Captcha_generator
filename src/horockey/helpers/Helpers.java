...
public class Helpers {
	...
	public static int brightness(int pix){
	...
	public static boolean isBorderPixel(BufferedImage img, int brightnessThreshold, int borderEpsilon, int x0, int y0){
		...
		if(Helpers.brightness(img.getRGB(x0, y0)) < brightnessThreshold){
			return false;
		}
		for(int dist = 1; dist <= borderEpsilon; dist++){
			...
			if(x0-dist >= 0 && y0-dist >= 0) {
				...
				for (int x = x0 - dist; x <= x0 + dist && x0 + dist < img.getWidth(); x++) {
					if (Helpers.brightness(img.getRGB(x, y0 - dist)) < brightnessThreshold) {
						return true;
					}
				}
				//Scan left vertical border
				for (int y = y0 - dist; y < y0+dist && y0+dist < img.getHeight(); y++){
					if(Helpers.brightness(img.getRGB(x0-dist, y)) < brightnessThreshold){
						return true;
					}
				}
			}
	...
	public static Point polarToEuclidian(Point p0, double phi, double r){
		int x = p0.x + (int)(r*Math.cos(phi));
		int y = p0.y + (int)(r*Math.sin(phi));
